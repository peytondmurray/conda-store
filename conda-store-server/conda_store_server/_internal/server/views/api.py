...
async def api_post_specification(
    ...
    """Create or update an environment from a specification yaml or lockfile.

    Parameters
    ----------
    request : Request
        Web request that hit this endpoint; contains information about the identity of the
        requester
    conda_store : app.CondaStore
        The running conda-store application
    auth : Authentication
        Authentication instance for the request. Used to authorize the request to
        build the environment
    entity : AuthenticationToken
        Token of the user making the request
    specification : str
        Can be either:
            - A YAML-formatted set of dependencies for the environment, along with some
              environment metadata; see
              https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html
              for details
            - A lockfile of an environment generated by conda-lock
    namespace : Optional[str]
        Namespace in which the environment is to be created. If unspecified, the primary
        namespace is used if the user is logged in, otherwise the default conda-store
        namespace is used
    is_lockfile : Optional[bool]
        Set to true if the specification is a lockfile, False otherwise
    environment_name : Optional[str]
        Name of the environment; only used if the specification is a lockfile, as the
        lockfiles do not store environment names
    environment_description : Optional[str]
        Description of the environment; only used if the specification is a lockfile,
        as lockfiles do not store environment descriptions

    Returns
    -------
    Dict[str, Union[str, Dict[str, int]]]
        Response containing the status and build ID for the new environment
    """
    with conda_store.get_db() as db:
...
async def api_put_build(
    ...
    """Rebuild the environment that was built for the given build ID.
    ...
    auth : Authentication
        Authentication instance for the request. Used to authorize the request to
        rebuild the given environment
    build_id : int
        Build ID for the environment to be rebuilt
    request : Request
        Web request that hit this endpoint; contains information about the identity of the
        requester
    ...
    -------
    Dict[str, Union[str, Dict[str, int]]]
        Response containing the status and messsage about the requested action, and the ID of
        the new build.
    """
    with conda_store.get_db() as db:
        build = api.get_build(db, build_id)
